...
class Block(nn.Module):
    ...
    def forward(self, x, bool_masked_pos=None):
        if self.gamma_1 is None:
            ...
            x = x + self.drop_path(
                self.gamma_1 * self.attn(self.norm1(x), bool_masked_pos)
            )
...
class VisionTransformerEncoder(nn.Module):
    ...
    def build_2d_sincos_position_embedding(
        self, embed_dim=768, temperature=10000.0, use_cls_token=False
    ):
        ...
        assert (
            embed_dim % 4 == 0
        ), "Embed dimension must be divisible by 4 for 2Dself.encoder.bui sin-cos position embedding"
        pos_dim = embed_dim // 4
    ...
    def build_1d_sincos_position_embedding(
        self, embed_dim=768, temperate=10000.0, use_cls_token=False
    ):
        h, w = self.patch_embed.patch_shape
        num_patches = h * w
        pos_x = torch.arange(num_patches, dtype=torch.float32)

        omega = torch.arange(0, embed_dim, step=2, dtype=torch.float32) / embed_dim
        encoding = torch.zeros(num_patches, embed_dim)
        encoding.requires_grad = False
        encoding[:, 0::2] = torch.sin(pos_x / (temperate**omega))
        encoding[:, 1::2] = torch.cos(pos_x / (temperate**omega))

        if not use_cls_token:
            pos_embed = nn.Parameter(encoding)
        else:
            pe_token = torch.zeros([1, 1, embed_dim], dtype=torch.float32)
            pos_embed = nn.Parameter(torch.cat([pe_token, encoding], dim=1))
        return pos_embed
...
class VisionTransformerNeck(nn.Module):
    ...
    def forward(
        ...
        for blk in self.regressor_blocks:
            x_masked = blk(
                x_masked,
                torch.cat([x_unmasked, x_masked], dim=1),
                pos_embed_masked,
