...
class VisionTransformerForMaskedImageModeling(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.pretext_neck = VisionTransformerNeck(
            patch_size=patch_size,
        ...
        if args.decoder_embed_dim != embed_dim:
            self.encoder_to_decoder = nn.Linear(
                embed_dim, args.decoder_embed_dim, bias=True
            )
    ...
    def forward(self, x, bool_masked_pos, return_all_tokens=None):
        ...
        pos_embed = (
            self.encoder.build_1d_sincos_position_embedding(dim, use_cls_token=True)
            .expand(batch_size, self.num_patches + 1, dim)
            .cuda(x_unmasked.device)
        )
        ...
        logits, latent_pred = self.pretext_neck(
            x_masked, x_unmasked, pos_embed_masked, pos_embed_unmasked, bool_masked_pos
        )
...
@register_model
def cae_small_patch48x4_48x432_8k_vocab(pretrained=False, **kwargs):
    model = VisionTransformerForMaskedImageModeling(
        img_size=(48, 432),
        patch_size=(48, 4),
        embed_dim=384,
